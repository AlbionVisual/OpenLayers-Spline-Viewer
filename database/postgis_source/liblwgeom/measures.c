	...
	for (i = 0; i < n1; i++)
	{
		...
		for (j = 0; j < n2; j++)
		{
			...
			if ((dl->mode != DIST_MAX) && (!lw_dist2d_check_overlap(g1, g2)) &&
                // AlbionVisual2026
			    (g1->type == LINETYPE || g1->type == CURVETYPE || g1->type == POLYGONTYPE || g1->type == TRIANGLETYPE) &&
			    (g2->type == LINETYPE || g2->type == CURVETYPE || g2->type == POLYGONTYPE || g2->type == TRIANGLETYPE))
			{
