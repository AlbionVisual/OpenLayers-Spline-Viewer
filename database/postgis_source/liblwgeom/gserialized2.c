...
// AlbionVisual2026
static size_t gserialized2_from_lwcurve(const LWLINE *line, uint8_t *buf)
{
	uint8_t *loc;
	int ptsize;
	size_t size;
    ...
    {
        lwerror("gserialized2 (925): Curve must have exactly 3 or 4 points, got %u", line->points->npoints);
        return 0;
    }
...
static size_t gserialized2_from_lwgeom_any(const LWGEOM *geom, uint8_t *buf)
{
    ...
    // AlbionVisual2026
    case CURVETYPE:
        return gserialized2_from_lwcurve((LWLINE *)geom, buf);
	case LINETYPE:
...
// AlbionVisual2026
static LWLINE *
lwcurve_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *size, int32_t srid)
{
	uint8_t *start_ptr = data_ptr;
	LWLINE *line;
	uint32_t npoints = 0;
    ...
    {
        lwfree(line);
        lwerror("gserialized2 (1388): Curve must have exactly 3 or 4 points, got %u", npoints);
        return NULL;
    }
...
lwgeom_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *g_size, int32_t srid)
{
    ...
    // AlbionVisual2026
    case CURVETYPE:
        return (LWGEOM *)lwcurve_from_gserialized2_buffer(data_ptr, lwflags, g_size, srid);
	case LINETYPE:
