...
// AlbionVisual2026
static size_t gserialized1_from_lwcurve(const LWLINE *line, uint8_t *buf)
{
	uint8_t *loc;
	int ptsize;
	size_t size;
...
static size_t gserialized1_from_lwgeom_any(const LWGEOM *geom, uint8_t *buf)
{
    ...
    // AlbionVisual2026
    case CURVETYPE:
		return gserialized1_from_lwcurve((LWLINE *)geom, buf);
	case LINETYPE:
...
// AlbionVisual2026
static LWLINE* lwcurve_from_gserialized1_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *size)
{
	uint8_t *start_ptr = data_ptr;
	LWLINE *line;
	uint32_t npoints = 0;
...
LWGEOM* lwgeom_from_gserialized1_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *g_size)
{
    ...
    // AlbionVisual2026
    case CURVETYPE:
        return (LWGEOM *)lwcurve_from_gserialized1_buffer(data_ptr, lwflags, g_size);
	case LINETYPE:
