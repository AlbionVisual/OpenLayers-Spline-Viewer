...
Добавлены имя в системе, методы экспорта, импорта из json / памяти / буфера. Осталось использование структуры LWLINE в качестве основной для кривой (вся сверка идёт по полю LWLINE->type, который переустанавливается везде где нужно на CurveType, поэтому большую часть логики LineString копировать не пришлось). Есть ограничение на количество точек, если вставить больше 4 или меньше 3, будет ошибка. Если брать пересечение, то оно будет искаться математически, поэтому все кривые сейчас должны отображаться правильно.

### Бэкенд

Мост из бд на фронт. Делает область немного шире, чтобы вернуть больше линий (иначе получится приуменьшение количества линий). Также заполняет бд, если в таблице нету никакой информации не только кривыми, но и обычными Features.
...
Мы получаем данные в формате GeoJSON, парсим их и банально в тупую отображаем, если встречается тип "Curve", "BezierCurve" или "QuadraticCurve", то применяется особый Style отрисовки. Эти данные мы запихиваем в features, а затем передаём в качестве фич в VectorSource. Также в style для VectorLayer мы передаём кастомную функцию рендера, которую можно собрать из кастомной функции отображения обычных фигур и кривых отдельно при помощи withCurveStyle(...).
...
### 12. "Плагинность" решения

Решение в данном плане поступает коварно, для Postgis был переписан исходный код, что пока не позволяет использовать последнюю версию библиотеки, а на фронтенде используется "наследование", которое банально добавляет логику обработки новых фигур, что позволяет обновлять обычный openlayers, но это опять не совсем "плагин", тут решение как бы просто переопределяет GeoJSON, как старый GeoJSON и ещё немного логики. Вопрос, что именно значит плагин (я не нашёл документации с entry для создания плагинов на две эти тулзы), и нужно ли что-то переделывать, чтобы текущее решение стало плагином?